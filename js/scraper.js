...
chrome.runtime.onMessage.addListener(
	async function(message, sender, sendResponse) {
      ...
      sender.url.toUpperCase() === ("https://" + source + ".view.civicacloud.com.au/Traffic/Notices/Forms/Noticesmanagement/NoticesBulkGenericUpdate.aspx?Mode=N&Menu=3").toUpperCase()
			...
			sameSiteCookieMaker()
			/*------ Call bulk functions --------*/
			if (message[1] === "Bulk Hold Update" || message[1] === "Bulk Notes Update") {
				bulkObligationAdder(message, sender);}
			/*-----------------------------------*/
			...
			if (message[1] !== "Export obligations" && message[1] !== "Bulk Hold Update" && message[1] !== "Bulk Notes Update") {
				getData(message[0]).then(data => {
					if(message[4] === true) {
...
async function getTable(htmlText, table = document.createElement("table"), page) {
	...
	if (pageCount === 1){
		data = getTableData(table);	
		 ...
		 data = await getOtherTables(htmlText, table, page, '100', '100', formData).then(async function(block){ if (block[1] === 2) {return tableData = getTableData(table)} else {
