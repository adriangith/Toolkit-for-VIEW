...
import { table } from "./tablemaker";
// need to import Config from ./config.js
...
async function getDebtorData() {
  const htmlText = await getPage(config[3])
  const debtorDetails = parsePage(config[3], htmlText)
  return debtorDetails
}

async function scrape(obligations: letterDataProps[]) {
  const array: typeof config = []
  let obligationsCount = obligations.length
  const obligationsCountFixed = obligations.length
  let data;
  for (const obligation in obligations) {
    data = await switchObligations(obligations[obligation]).then(async function () {
      const promises = config.filter(function (page) {
        ...
        let data;
        let htmlText = await getPage(page)
        if (page.controlID !== undefined) {
          const formData = await getFormData(htmlText);
          formData.set(page.controlID + '.x', String(0));
          formData.set(page.controlID + '.y', String(0));
      ...
      data = Promise.all<Promise<typeof config[0]>[]>(promises).then(dataArray => {
        const data = Object.assign({}, ...dataArray) as typeof config[0]
        array.push(data);
        return array;
      })
      return await data
...
async function parsePage(page, htmlText) {
  ...
  for (const item in page.targetTags) {
    const item = page.targetTags[item];
    if (item.method === 'text') {
...
function getAppData(data) {
  chrome.storage.local.get(['value'], function (items) {
    ...
    for (const application in applicationData) {
...
async function getTable(htmlText, table = document.createElement("table"), page) {
  const block = await combineTable(htmlText, table)
  let data;
  const formData = block[0]
  const pageCount = block[1]
  if (pageCount === 1) {
...
function getTableData(table) {
  ...
  let blata;
  const filter = "642";
  const filter2 = "Warrant Issue Fee added in VIMS";
  const tr = table.children[0].childNodes;
  for (j = 0; j < tr.length; j++) {
    const td = tr[j].childNodes;
    const cell = td[5];
    if (cell) {
...
async function getFormData(htmlText) {
  const theForm = htmlText.filter('form')[0]
...
function transformations(data, type) {
  ...
  if (data.Address_1 !== undefined) {
    ...
    data.Address_1 = data.Address_1
      ...
      .replace(new RegExp(' Ct$| Ct $ | Ct {2}$'), ' Court')
...
async function getData(obligations: letterDataProps[]) {
  if (running === true) {
  ...
  const grabbed = await scrape(obligations);
