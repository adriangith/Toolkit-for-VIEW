...
interface ApplicationFormData {
    ...
    altAddress: AddressData;
    [key: string]: boolean | string | AddressData;
}
...
interface Option {
    value: string;
    label: string;
    revealedField?: FieldConfig;
}

interface RowConfig {
    type: 'fieldRow' | 'addressBlock' | 'radioGroup';
    id?: string;
    label?: string;
    fields?: FieldConfig[];
    options?: Option[];
    renderCondition?: (formData: ApplicationFormData) => boolean;
    disabledCondition?: (formData: ApplicationFormData) => boolean;
}

const AddressBlock = ({ addressData, onAddressChange, isDisabled = false }: { addressData: AddressData, onAddressChange: (e: ChangeEvent<HTMLInputElement>) => void, isDisabled?: boolean }) => {
    const addressFields = [
...
const FormField = ({ config, value, onChange, isDisabled = false }: { config: FieldConfig, value: boolean | string, onChange: (e: ChangeEvent<HTMLInputElement>) => void, isDisabled?: boolean }) => {
    const commonProps = {
        id: config.id,
        tabIndex: 60,
        onChange: onChange,
...
const formConfig: { rows: RowConfig[] } = {
    rows: [
        {
            type: 'fieldRow',
            fields: [
        ...
        },
        {
            ...
            renderCondition: (formData) => formData.isThirdParty === true
        },
        {
            type: 'addressBlock',
            id: 'mainAddress',
        ...
        },
        {
            ...
            renderCondition: (formData) => formData.isThirdParty === true && formData.addressTo === 'Alt 3rd Party',
...
function ApplicationForm() {
    ...
    useEffect(() => {
        try {
            const id = (document.querySelector("#DebtorDetailsCtrl_DebtorIdSearch") as HTMLInputElement)?.defaultValue;
    ...
    const debouncedSave = useMemo(() => debounce((id: string, data: ApplicationFormData) => {
    ...
    useEffect(() => {
        ...
        if (targetElement && targetElement.parentNode && !document.getElementById(buttonId)) {
    ...
    const handleStateChange = (id: string, value: string | null, type = 'value') => {
        setFormData(prev => ({
            ...prev,
            [id]: type === 'checkbox' ? !prev[id] : (value as string | boolean)
        }));
    };

    const handleAddressChange = (addressType: string) => (e: ChangeEvent<HTMLInputElement>) => {
        const { id, value } = e.target;
        setFormData(prev => ({
            ...prev,
            [addressType]: { ...(prev[addressType] as AddressData), [id]: value }
        }));
    };
                    ...
                    {formConfig.rows.map((row, index) => {
                        ...
                        switch (row.type) {
                                ...
                                return (
                                    <tr key={index}>
                                        ...
                                        <td className="table-value-column" style={{ verticalAlign: 'middle' }}>
                                            <FormField
                                                config={row.fields![0]}
                                                value={formData[row.fields![0].id] as boolean}
                            ...
                            case 'addressBlock':
                                ...
                                return <AddressBlock key={index} addressData={formData[row.id!] as AddressData} onAddressChange={handleAddressChange(row.id!)} isDisabled={isDisabled} />;
                                                ...
                                                <div key={opt.value}>
                                                    ...
                                                    {opt.revealedField && formData[row.id!] === opt.value && (
                                                        <div style={{ marginLeft: '25px', marginTop: '5px', display: 'flex', alignItems: 'center' }}>
                                                            <FormField config={opt.revealedField} value={formData[opt.revealedField.id] as boolean} onChange={() => handleStateChange(opt.revealedField!.id, null, 'checkbox')} />
